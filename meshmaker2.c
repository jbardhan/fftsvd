...
#define MESHSES_PATH PWD"/Users/b55323/bin/frentos/meshses3.prl"
#define MESHSAS_PATH PWD"/Users/b55323/bin/frentos/meshsas3.prl"
#define REMESH_PATH PWD"/Users/b55323/bin/frentos/remesh.prl"

#define CONNECTED_GST_TOR_PATH PWD"/Users/b55323/bin/frentos/connectedgsttor"
...
unsigned int isInside(Panel* inpanels, unsigned int numinpaanels, Panel* outpanels, unsigned int numoutpanels) {
   ...
   if (fabs(sum + 4.0*M_PI) < 1e-1)
      ...
      printf("isInside Futz, returning OKAY because this is the silly SoftWAXS mesher!\n");
      return 1;
   }
}
...
void generateMSMSflat(unsigned int accessible, Panel*** surfpanels, unsigned int** numsurfpanels, unsigned int* numsurfs, char*** surffilenames, Panel*** cavitypanels, unsigned int** numcavitypanels, unsigned int* numcavities, char*** cavityfilenames) {
   ...
   sprintf(command, "%s -no_header -probe_radius %f -density %f -if %s -of %s > %s",
           msmspath, (accessible ? 1.4 : proberadius), (accessible ? saltdensity : dielectricdensity), xyzrfilename, surfacefilename, outputfilename);
