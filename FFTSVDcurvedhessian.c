...
int main(int argc, char* argv[]) {
   ...
   Vector phi, curchargevec, Born_radii;
...
*/
   ...
   Born_radii = Vector_allocate(charge->numcharges);
   printf("Self-energy:  Born radius:\n");
   ...
   if (0) {
      ...
      Born_radii[colcount] = 1 / ((phi[colcount]/2) * 2 /( CONVERSION * (1/odiel - 1/idiel)) );
      printf("atom  #%d  %lf \t %lf \n", colcount, phi[colcount]/2, Born_radii[colcount] );
   }
   }
...
/*    printf("ALLCOUNT: %u\n", allcount); */
/*    printf("QUADCOUNT: %u\n", quadcount); */
   Vector_free(Born_radii);
   Vector_free(curchargevec);
